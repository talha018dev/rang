    ...
    <div class="men-page pb-30! sm:pb-0!">
      ...
      <section class="hero-banner">
        <div class="hero-content">
          <div class="hero-text">{{ campaignTitle }}</div>
          <NuxtImg :src="heroImage" :alt="`${campaignTitle} products`" class="hero-img" loading="eager" format="webp"
            quality="90" />
        </div>
      </section>
...
// Format campaign ID to title
const campaignTitle = computed(() => {
  if (!campaignId.value) return ''
  // Decode URL-encoded campaign and format it
  const decoded = decodeURIComponent(campaignId.value)
...
// Meta
useHead({
  title: `${campaignTitle.value} - Rang`,
  meta: [
    { name: 'description', content: `Discover products from the ${campaignTitle.value.toLowerCase()} campaign.` }
  ]
})
...
const fetchProducts = async () => {
  ...
  try {
    // Build API URL with campaign parameter
    const { backendUrl } = useApi()
    let apiUrl = `${backendUrl}/product?sort=${selectedSort.value}&page=${currentPage.value}&campaign=${encodeURIComponent(campaignId.value)}`
    ...
    if (err?.statusCode === 404 || err?.status === 404 || err?.response?.status === 404) {
      error.value = 'No products found for this campaign'
    } else {
...
watch(campaignId, () => {
  currentPage.value = 1 // Reset to first page when campaign changes
  fetchProducts()
})
...
<style scoped>
/* Override AppHeader navigation colors for campaigns page */
:global(.nav-link) {
...
/* Override hamburger icon color for campaigns page */
